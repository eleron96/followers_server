...
from src.database.medium_db import add_medium_data, get_latest_medium_data, \
    get_followers_over_period
from src.utils.logger import setup_logger
from src.database.medium_db import get_daily_followers
...
def get_followers_apollo(html):
    try:
        ...
        if match:
            ...
            user_key = next((k for k in apollo_data if
                             'User:' in k and f'@{Config.MEDIUM_USERNAME}' in k),
                            None)
            if user_key:
...
def get_medium_followers():
    try:
        ...
        followers = get_followers_apollo(html_content) or get_followers_direct(
            html_content)
...
def get_medium_daily_stats(username):
